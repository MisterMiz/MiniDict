...
class _MinitranState extends State<Minitran> {
  ...
  search(value) async {
    _query = await helper.searchEng2Th(value);
    // print(_query);
    setState(() {
      indexe2t = _query.length;
      word = _query[0]['esearch'];
      sttentry = _query[0]['tentry'];
      steentry = value;
      // print(steentry);
      eword = word;
      // print('tword $tword\neword $eword');
      // print('tentey $sttentry\neentey $steentry');
    });
  }

  searcht2e(value) async {
    t2e = await helper.searchTh2Eng(value);
    // print(t2e);
    setState(() {
      indext2e = t2e.length;
      word = t2e[0]['tsearch'];
      steentry = t2e[0]['eentry'];
      sttentry = value;
      // print(sttentry);
      tword = word;
      // print('tword $tword\neword $eword');
                ...
                Expanded(
                  ...
                  child: Container(
                    child: IconButton(
                      ...
                      onPressed: () => checkLang(_textE2T.text),
                    ),
                  ),
                )
              ],
